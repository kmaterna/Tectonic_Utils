...
Variable "b" is the unit vector toward the reference strike azimuth in a coordinate system in which the +Y axis is north, 
	the +X axis is east, and the +Z axis is up.  
...
Variable "i" is the basis vector for the X axis of the nodal plane coordinate system.  If the rake is a negative number, 
	the strike vector is the X axis and the dip vector is the Y axis.  If the rake is a positive vector, the dip vector is the X axis 
	and the strike vector is the Y axis.  Variable "j" is the basis vec  tor for the Y axis of the nodal plane coordinate system.  
	Variable "d" is the basis vector of the Z axis of the nodal plane coordinate system.  
Variable "k" is the unit vector in the nodal plane coordinate system that is coincident with the slip vector on the (input) nodal plane.  
Variable "m" is the transformation matrix from the nodal plane coordinate system to the geographic coordinate system.  
Variable "rakeVector" is the unit location vector along the slip direction on the nodal plane, expressed in the geographic coordinate system.  
Variable "n" is the dip vector of the auxilary plane, expressed in radian measure.  
Variable "s" is the unit vector projection of the rake vector onto the X - Y (horizontal) plane of the geographic coordinate system.
Explanation of the result.  The first element is the trend of the dip vector of the auxillary plane, in degrees.  
	The second element is the dip angle of the auxillary plane, in degrees.  The third element is the trend of the slip vector 
	of the auxillary plane, in degrees.
"""
...
def vector_angle(v1, v2):
	vectorAngle = np.arccos(np.dot(v1, v2)/(vector_norm(v1)*vector_norm(v2)));
	# Returns in radians
	return vectorAngle;
...
def find_aux_plane(strike, dip, rake):
	...
	# Start by computing a few important quantities.
	a = strike;  # degrees
